...
export type Screen =
  | 'welcome' | 'warn' | 'seed' | 'verify' | 'import' | 'pin' | 'bio'
...
interface State {
  ...
  pin: string; pinFirst: string; pinError: string; faceId: boolean; autoLock: number; currency: Currency;
...
function initialState(start: WalletProps['startScreen']): State {
  return {
    ...
    pin: '', pinFirst: '', pinError: '', faceId: true, autoLock: 5, currency: 'EUR',
...
export function useWallet(P: WalletProps) {
  ...
  const pinDone = (p: string) => {
    ...
    if (s.pinMode === 'confirm') {
      if (p === s.pinFirst) return setState({ screen: 'bio', pin: '' });
  ...
  const pinPress = (k: string) => {
    ...
    if (k === 'bio') { if (S.pinMode === 'unlock' && S.faceId) setState({ screen: 'home', tab: 'home', pin: '' }); return; }
  ...
  const pinSubs: Record<PinMode, string> = {
    ...
    unlock: S.faceId ? 'PIN eingeben oder Face ID nutzen.' : 'PIN eingeben.', reveal: 'Für die Phrase ist die PIN nötig. Face ID reicht hier nicht.'
  };
  const pinDots = [0, 1, 2, 3, 4, 5].map((i) => (i < S.pin.length ? IND : '#2A2A32'));
  const showBio = S.pinMode === 'unlock' && S.faceId;
  const pinKeys = ['1', '2', '3', '4', '5', '6', '7', '8', '9', showBio ? 'bio' : '', '0', 'del'].map((k) => ({
    label: k === 'bio' || k === 'del' ? '' : k,
    icon: (k === 'bio' ? <Icon name="faceid" color={ACC} size={28} weight={1.8} /> : k === 'del' ? <Icon name="del" color={INK} size={24} weight={1.9} /> : null) as ReactNode,
    bg: k && k !== 'bio' && k !== 'del' ? '#141418' : 'transparent',
    onClick: () => pinPress(k)
  }));
  ...
  const confirmCta = S.faceId ? 'Mit Face ID bestätigen' : 'Mit PIN bestätigen';
  ...
  return {
    ...
    bioYes: () => { setState({ faceId: true, screen: 'home', tab: 'home' }); flash(S.flow === 'import' ? 'Konten abgeleitet · Bitcoin-Scan abgeschlossen' : 'Wallet bereit'); },
    bioNo: () => { setState({ faceId: false, screen: 'home', tab: 'home' }); flash('Wallet bereit'); },
    ...
    faceId: S.faceId, toggleFace: set({ faceId: !S.faceId }),
    ...
    // overlays / chrome
    confirmOpen: S.confirmOpen, confirmLabel: S.faceId ? 'Face ID' : 'PIN bestätigt', runConfirm,
